...
app.MapGet("/songs", async (HttpContext context) =>
{
    string json = "";
...
app.MapGet("/play/{id}", async (HttpContext context) =>
{
    try
    {
        ...
        Task c = FFmpegWrapper.GetFFmpegWrapper()
            .customCommandTest($" -i  ./bin/Mp4Files/{songName}.mp4 -s 140x80 -pix_fmt rgba -f image2pipe -vcodec png -");
